...
async function deployDEX() {
  ...
  console.log(
    `⛓️  Network: 0G Aristotle (Chain ID: ${(await provider.getNetwork()).chainId})\n`,
  );
  ...
  try {
    ...
    console.log(
      "  1. Download UniswapV2Factory.sol from https://github.com/Uniswap/v2-core",
    );
    ...
    console.log(
      "  npx hardhat run scripts/deploy-dex-router.ts --network 0g\n",
    );
    ...
    console.log(
      "🔍 Checking for existing Uniswap V2 deployment on 0G Aristotle...\n",
    );
