...
function updateEnv(updates: Record<string, string>) {
  let content = fs.existsSync(".env.launch")
    ? fs.readFileSync(".env.launch", "utf-8")
    : "";
...
async function deploy() {
  ...
  if (!oinioAddress) {
    log(
      "Step 3/3: Skipping pool creation (OINIO_TOKEN_ADDRESS not provided)",
      "WARN",
    );
  } else {
    ...
    try {
      ...
      log(
        `OINIO balance: ${ethers.formatUnits(oinioBalance, oinioDecimals)}`,
        "INFO",
      );
      log(
        `WGAS balance: ${ethers.formatUnits(wgasBalance, wgasDecimals)}`,
        "INFO",
      );
