...
async function withdrawToAristotle() {
  try {
    ...
    console.log('🔗 Transaction Ref:', response.result?.ref || 'Check Kraken dashboard');
...
async function main() {
  ...
  if (!hasBalance) {
    console.log('❌ Cannot proceed with withdrawal due to insufficient balance');
    process.exit(1);
  }
  ...
  if (success) {
    ...
    console.log('📊 Check balance at: https://chainscan.0g.ai/address/' + DESTINATION_ADDRESS);
