...
async function main() {
  ...
  console.log("Minted", ethers.formatEther(initialSupply), "OINIO to", deployer.address);

  // Deploy QuantumPiSovereignStaking
  console.log("🏛️ Deploying QuantumPiSovereignStaking...");
  const QuantumPiSovereignStaking = await ethers.getContractFactory("QuantumPiSovereignStaking");
  ...
  const staking = await QuantumPiSovereignStaking.deploy(oinioAddress, trustedForwarder);
