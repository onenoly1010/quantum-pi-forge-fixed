...
function addToIPFS() {
  ...
  try {
    ...
    const result = execSync(`curl -X POST -F "path=@${BUILD_DIR}" "${IPFS_CONFIG.api}/api/v0/add?recursive=true&wrap-with-directory=true"`, {
      encoding: 'utf8'
    });
    ...
    execSync(`curl -X POST "${IPFS_CONFIG.api}/api/v0/pin/add?arg=${ipfsHash}"`, { stdio: 'pipe' });
...
function testIPFSGateway(ipfsHash) {
  ...
  try {
    // Test local gateway
    execSync(`curl -s "${IPFS_CONFIG.gateway}/ipfs/${ipfsHash}" | head -c 100`, { stdio: 'pipe' });
    log('Local IPFS gateway accessible', 'success');

    // Test public gateway (may take time to propagate)
    log('Note: Public gateway access may take a few minutes to propagate globally');
...
async function main() {
  ...
  console.log('2. Run the Kraken withdrawal script: node scripts/secure-kraken-withdrawal.js');
