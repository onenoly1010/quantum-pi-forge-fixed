...
async function main() {
  ...
  const contractAddressPath = path.join(__dirname, '..', '..', '.contract_address');
  ...
  const ipfsHash = fs.existsSync(ipfsHashPath) ? fs.readFileSync(ipfsHashPath, 'utf8').trim() : "";
  const radicleRepoId = fs.existsSync(radicleRepoPath) ? fs.readFileSync(radicleRepoPath, 'utf8').trim() : "";
  ...
  const CentralAwarenessV2 = await ethers.getContractFactory("CentralAwarenessV2");
  ...
  if (isReady) {
    ...
    console.log("⚡ The 1010 Hz resonance flows through decentralized channels");
  }
}
