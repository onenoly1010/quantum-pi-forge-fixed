...
describe("Pi Network Authentication", () => {
  ...
  describe("Development Mode (No Pi SDK)", () => {
    ...
    it("shows development mode message initially", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            "🔧 Development mode - Click to see demo simulation",
          ),
        ).toBeInTheDocument();
      });
    });
    ...
    it("shows demo simulation when connecting without Pi SDK", async () => {
      ...
      expect(
        screen.getByText("🔧 Demo mode: Simulating Pi connection..."),
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByText(
          "✅ [DEMO] Connected as @demo_user - Deploy to Pi Network for real authentication",
        ),
      ).toBeInTheDocument();
    });
    ...
    it("shows development preview notice when Pi SDK is not available", () => {
      ...
      expect(
        screen.getByText(/This demo simulates the Pi connection/),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Pi Browser Mode (With Pi SDK)", () => {
    ...
    it("detects Pi SDK and shows ready status", () => {
      ...
      expect(
        screen.getByText("Pi SDK detected. Click to connect."),
      ).toBeInTheDocument();
    });
    ...
    it("calls Pi.init and Pi.authenticate when connecting", async () => {
      ...
      await waitFor(() => {
        expect(mockPi.init).toHaveBeenCalledWith({
          version: "2.0",
          sandbox: false,
        });
    ...
    it("shows connected status with username on successful auth", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("✅ Connected as @sovereign_pi_user"),
        ).toBeInTheDocument();
      });
    });
    ...
    it("shows error message when authentication fails", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("❌ Auth canceled or failed: User cancelled"),
        ).toBeInTheDocument();
      });
    });
    ...
    it("handles unknown authentication errors gracefully", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("❌ Auth canceled or failed: Unknown error"),
        ).toBeInTheDocument();
      });
    });
    ...
    it("shows connecting status while authenticating", async () => {
      ...
      expect(
        screen.getByText("Connecting to Pi Network..."),
      ).toBeInTheDocument();
    });
  });
