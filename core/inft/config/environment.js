...
const inftConfig = {
  ...
  memory: {
    ...
    consolidationThreshold:
      parseInt(process.env.CONSOLIDATION_THRESHOLD) || 100,
    retentionRatio: parseFloat(process.env.MEMORY_RETENTION_RATIO) || 0.8,
    importanceThreshold:
      parseFloat(process.env.MEMORY_IMPORTANCE_THRESHOLD) || 0.3,
  },
  ...
  metadata: {
    ...
    animationBaseURI:
      process.env.ANIMATION_BASE_URI ||
      "https://api.quantumpiforge.com/animations/",
...
function validateINFTConfig() {
  ...
  if (
    inftConfig.contracts.hybridNFT &&
    !inftConfig.contracts.hybridNFT.startsWith("0x")
  ) {
  ...
  if (
    inftConfig.personality.minCoherence >= inftConfig.personality.maxCoherence
  ) {
    errors.push("Min coherence must be less than max coherence");
  }

  if (
    inftConfig.evolution.maxTraitsChange <= 0 ||
    inftConfig.evolution.maxTraitsChange > 1
  ) {
