...
class ResponseGenerator {
  ...
  async generateResponse(inftId, interactionType, context = {}) {
    try {
      ...
      const enhancedResponse = await this.enhanceWithMemories(
        baseResponse,
        memories,
        personality,
      );
      ...
      // Add emotional depth
      const emotionalResponse = this.addEmotionalDepth(
        enhancedResponse,
        personality,
        context,
      );
  ...
  async personalizeResponse(inftId, response, interactionType) {
    ...
    const recentResponses = history
      .filter((h) => h.interactionType === interactionType)
      .slice(-5);
  ...
  calculateConfidence(personality, memories, context) {
    ...
    if (memories.length > 0) {
      const avgRelevance =
        memories.reduce((sum, m) => sum + (m.relevanceScore || 0.5), 0) /
        memories.length;
