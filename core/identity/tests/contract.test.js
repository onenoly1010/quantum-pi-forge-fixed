...
describe("OINIO Identity System", () => {
  ...
  describe("SoulRegistry", () => {
    it("should mint a new soul", async () => {
      ...
      await expect(
        soulRegistry.connect(oracle).mintSoul(piUid, user1.address),
    ...
    it("should transfer soul ownership", async () => {
      ...
      await soulRegistry
        .connect(user1)
        .transferSoul(soul.soulId, user2.address);
  ...
  describe("OGToken", () => {
    it("should grant OG status", async () => {
      ...
      expect(await ogToken.balanceOf(user1.address)).to.equal(
