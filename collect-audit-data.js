...
async function collectAuditData() {
  ...
  try {
    ...
    auditData.pairAddress = await factoryContract.getPair(
      CONFIG.OINIO_TOKEN,
      CONFIG.WETH_ADDRESS,
    );
  ...
  if (
    auditData.pairAddress &&
    auditData.pairAddress !== "0x0000000000000000000000000000000000000000"
  ) {
    try {
      // Get pair data
      ...
      auditData.deadLPBalance = await pairContract.balanceOf(
        CONFIG.DEAD_ADDRESS,
      );
      ...
      console.log(
        `✅ LP Supply: ${ethers.formatEther(auditData.totalLPSupply)}`,
      );
      console.log(
        `✅ Dead Address LP: ${ethers.formatEther(auditData.deadLPBalance)}`,
      );
      console.log(
        `✅ Reserves: ${ethers.formatEther(reserve0)} / ${ethers.formatEther(reserve1)}`,
      );
    } catch (error) {
  ...
  console.log(
    `PAIR_ADDRESS=${auditData.pairAddress || "[INSERT_PAIR_ADDRESS]"}`,
  );
  console.log(`TOKEN_OWNER=${auditData.tokenOwner || "[INSERT_TOKEN_OWNER]"}`);
  console.log(
    `FACTORY_OWNER=${auditData.factoryOwner || "[INSERT_FACTORY_OWNER]"}`,
  );
  console.log(
    `TOTAL_SUPPLY=${auditData.totalSupply ? ethers.formatEther(auditData.totalSupply) : "[INSERT_TOTAL_SUPPLY]"}`,
  );
  console.log(
    `LP_TOTAL_SUPPLY=${auditData.totalLPSupply ? ethers.formatEther(auditData.totalLPSupply) : "[INSERT_LP_TOTAL_SUPPLY]"}`,
  );
  console.log(
    `DEAD_LP_BALANCE=${auditData.deadLPBalance ? ethers.formatEther(auditData.deadLPBalance) : "[INSERT_DEAD_LP_BALANCE]"}`,
  );

  console.log("\n🔗 VERIFICATION LINKS:");
  console.log(
    `Token Owner Check: ${CONFIG.EXPLORER_URL}/address/${CONFIG.OINIO_TOKEN}#readContract`,
  );
  console.log(
    `Factory Owner Check: ${CONFIG.EXPLORER_URL}/address/${CONFIG.DEX_FACTORY}#readContract`,
  );
  if (auditData.pairAddress) {
    console.log(
      `LP Burn Check: ${CONFIG.EXPLORER_URL}/address/${auditData.pairAddress}#readContract`,
    );
  }
  ...
  console.log(
    "[INSERT_FACTORY_RENOUNCE_TX_HASH] - Factory renounce transaction",
  );
