...
const OTLP_HTTP_ENDPOINT =
  process.env.OTLP_ENDPOINT || "http://localhost:4318/v1/traces";
const OTLP_GRPC_ENDPOINT =
  process.env.OTLP_GRPC_ENDPOINT || "http://localhost:4317";
...
export async function traceBlockchainTransaction<T>(
  ...
  return withTracing(`blockchain.${operationName}`, operation, {
...
export async function traceWalletInteraction<T>(
  ...
  return withTracing(`wallet.${operationName}`, operation, {
...
export async function traceAICall<T>(
  ...
  return withTracing(`ai.${modelName}`, operation, {
    "ai.provider": metadata?.provider || "unknown",
    "quantum.component": "AI",
    ...(metadata?.promptTokens && {
      "ai.prompt_tokens": metadata.promptTokens,
    }),
    ...(metadata?.completionTokens && {
      "ai.completion_tokens": metadata.completionTokens,
    }),
...
export async function traceAPIHandler<T>(
  ...
  return withTracing(`api.${method.toLowerCase()}.${endpoint}`, handler, {
    "http.method": method,
    "http.route": endpoint,
