...
function loadConfig() {
  return {
    ...
    // Database
    database: {
      url: getEnvVar('DATABASE_URL', 'mongodb://localhost:27017/quantumpiforge'),
    ...
    oracle: {
      ...
      maxReadingsPerHour: parseInt(getEnvVar('ORACLE_MAX_READINGS_PER_HOUR', '100'))
    },

    // iNFT
    inft: {
      maxEvolutionLevel: parseInt(getEnvVar('INFT_MAX_EVOLUTION_LEVEL', '10')),
      evolutionCooldown: parseInt(getEnvVar('INFT_EVOLUTION_COOLDOWN', '86400000')), // 24 hours
      metadataCacheTimeout: parseInt(getEnvVar('INFT_METADATA_CACHE_TIMEOUT', '3600000')), // 1 hour
      ipfsGateway: getEnvVar('IPFS_GATEWAY', 'https://gateway.pinata.cloud/ipfs/')
    },
    ...
    cors: {
      ...
      allowedHeaders: ['Content-Type', 'Authorization', 'X-Requested-With', 'X-Session-Token', 'X-Pi-Token', 'X-Soul-Signature']
    },
    ...
    monitoring: {
      ...
      healthCheckInterval: parseInt(getEnvVar('HEALTH_CHECK_INTERVAL', '30000')) // 30 seconds
    }
  };
}
...
function validateConfig(config = null) {
  ...
  if (!config.blockchain.polygon.contracts.soulRegistry) {
    warnings.push('SOUL_REGISTRY_ADDRESS not set - soul verification may not work');
  }
