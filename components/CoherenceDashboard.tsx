...
export default function CoherenceDashboardComponent({
  className = "",
}: CoherenceDashboardProps) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-300">
            Qualitative field states • Event-driven coherence • No metrics, no
            hierarchies
          </p>
          <button
              ...
              {nodes.map((node) => (
                <div key={node.id} className="bg-slate-700/50 rounded-lg p-3">
                  ...
                  <div className="flex gap-2 mt-1 text-sm">
                    <span className={getPresenceColor(node.presence)}>
                      {node.presence}
                    </span>
                    <span className={getIntegrityColor(node.integrity)}>
                      {node.integrity}
                    </span>
                    <span className={getConsentColor(node.consent)}>
                      {node.consent}
                    </span>
                  </div>
                </div>
              ...
              {relations.map((relation) => (
                ...
                >
                  ...
                  <div className="flex gap-2 mt-1 text-sm">
                    ...
                    <span className={getToneColor(relation.tone)}>
                      {relation.tone}
                    </span>
                  </div>
                </div>
              ...
              {motifs.map((motif) => (
                <div key={motif.id} className="bg-slate-700/50 rounded-lg p-3">
                  ...
                  <div className="flex gap-2 text-sm">
                    <span className={getMotifStateColor(motif.state)}>
                      {motif.state}
                    </span>
        ...
        <div className="mt-8 text-center text-slate-400 text-sm">
          Coherence Dashboard • Qualitative States Only • Event-Driven
          Architecture
        </div>
      </div>
    </div>
