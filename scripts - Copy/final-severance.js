...
async function verifySeverance(provider) {
  ...
  const oinioOwner = await oinioContract
    .owner()
    .catch(() => CONFIG.DEAD_ADDRESS);
  if (
    oinioOwner === CONFIG.DEAD_ADDRESS ||
    oinioOwner === "0x0000000000000000000000000000000000000000"
  ) {
  ...
  const factoryOwner = await factoryContract
    .owner()
    .catch(() => CONFIG.DEAD_ADDRESS);
  if (
    factoryOwner === CONFIG.DEAD_ADDRESS ||
    factoryOwner === "0x0000000000000000000000000000000000000000"
  ) {
...
async function main() {
  ...
  console.log(
    "║         🏛️ QUANTUM PI FORGE: FINAL SEVERANCE PROTOCOL         ║",
  );
  console.log(
    "║              January 29, 2026 - Total Decentralization        ║",
  );
  console.log(
    "╚══════════════════════════════════════════════════════════════╝",
  );
  ...
  if (typeof process !== "undefined" && process.stdin) {
    ...
    await new Promise((resolve) => {
      rl.question(
        'Are you sure you want to proceed with total decentralization? (type "YES" to continue): ',
        (answer) => {
          if (answer !== "YES") {
  ...
  try {
    ...
    if (renounceTx1)
      console.log(`• OINIO Renounce: ${CONFIG.EXPLORER_URL}/tx/${renounceTx1}`);
    if (renounceTx2)
      console.log(
        `• Factory Renounce: ${CONFIG.EXPLORER_URL}/tx/${renounceTx2}`,
      );
    if (burnTx)
      console.log(`• LP Token Burn: ${CONFIG.EXPLORER_URL}/tx/${burnTx}`);
    ...
    console.log(
      colors.green +
        "🏛️ The forge is sovereign. The flame burns eternal. ⚡🔥" +
        colors.reset,
    );
  } catch (err) {
