...
async function verifyContract(provider, address, name) {
  ...
  try {
    ...
    error(
      `Cannot connect to RPC or contract ${name} not found at ${address}: ${err.message}`,
    );
  }
}
...
async function verifyOINIOToken(provider) {
  ...
  try {
    const [name, symbol, decimals, totalSupplyRaw, owner] = await Promise.all([
      ...
      contract
        .owner()
        .catch(() => "0x0000000000000000000000000000000000000000"), // Might not have owner function
    ]);
    ...
    if (owner === "0x0000000000000000000000000000000000000000") {
      ...
      info(
        "Contract still has an owner - can be renounced for full decentralization",
      );
    }
  } catch (err) {
...
async function verifyDEXFactory(provider) {
  ...
  try {
    ...
    const [tokenA, tokenB] =
      CONFIG.OINIO_TOKEN.toLowerCase() < wethAddress.toLowerCase()
    ...
    if (pairAddress !== "0x0000000000000000000000000000000000000000") {
      ...
      info(
        `  Pool Reserves: ${ethers.formatEther(reserve0)} / ${ethers.formatEther(reserve1)}`,
      );
      success("Pool has liquidity!");
    } else {
      info(
        "OINIO/0G pair does not exist yet (will be created with first liquidity)",
      );
    }
  } catch (err) {
...
async function main() {
  ...
  console.log(
    "║         🔍 QUANTUM PI FORGE: CONTRACT VERIFICATION          ║",
  );
  console.log(
    "║              January 29, 2026 - Sovereign Audit              ║",
  );
  console.log(
    "╚══════════════════════════════════════════════════════════════╝",
  );
  ...
  try {
    ...
    if (network.chainId === BigInt(CONFIG.CHAIN_ID)) {
      ...
      warning(
        `Connected to different network. Expected ${CONFIG.CHAIN_ID}, got ${network.chainId}`,
      );
    }
    ...
    console.log(
      `• OINIO Token: ${CONFIG.EXPLORER_URL}/address/${CONFIG.OINIO_TOKEN}`,
    );
    console.log(
      `• DEX Router: ${CONFIG.EXPLORER_URL}/address/${CONFIG.DEX_ROUTER}`,
    );
    console.log(
      `• DEX Factory: ${CONFIG.EXPLORER_URL}/address/${CONFIG.DEX_FACTORY}`,
    );
    console.log("");

    console.log(
      colors.green + "✅ READY FOR LIQUIDITY ADDITION" + colors.reset,
    );
    console.log("");
